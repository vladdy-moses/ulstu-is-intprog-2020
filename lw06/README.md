...
Максимальный балл за сдачу вовремя можно получить до 13.12.2020 23:59:59 по Ульяновскому времени.

Обычный срок сдачи работы до 20.12.2020 23:59:59 по Ульяновскому времени.

## Об оформлении отчёта

В рамках выполнения лабораторной работы необходимо создавать скриншоты, демонстрирующие Ваши действия.
Таким образом крайне рекомендуется перед началом выполнения работы создать документ в любом текстовом процессоре (Microsoft Word или Google Docs) и делайте в нём скриншоты.
Не стоит делать скриншот каждого клика мышкой или каждой экранной формы, но должно быть понимание, что эти скриншоты делали Вы.
...
Веб-сервер - серверное программное обеспечение, которое служит для отображения пользователям данных в вебе.
...
Например, пользователи со всего Интернета могут обращаться к веб-серверу nginx, а он внутри перенаправляет (проксирует) запросы к другому веб-серверу (например, тому же Apache).
Такая связка используется очень много где и помогает распределить нагрузку и увеличить отказоустойчивость системы.

IIS - веб-сервер от Microsoft для запуска начинающих устаревать веб-прилоежний на технологиях этой корпорации.
...
> Если хотите получить графическую среду по управления СУБД, выберите дополнительно "PhpMyAdmin".
...
Выделяют класс таких систем - систем управления контентом.
...
Введите команду `ХХ`, чтобы задать пароль системному пользователю _root_:

!TODO_PIC!

Далее создадите БД при помощи команд `XX` и `XX`:

!TODO_PIC!

## Устновка WordPress

Для устновки WordPress скачайте архив с [официального сайта](https://ru.wordpress.org/) и распакуйте его в папку _XXX_:
...
Если в меню нажать на название сайта, то можно перейти на Ваш сайт (ссылка есть выше).
...
Заметьте, что это намного проще, чем делать верстать сайт с нуля.

> Помнится 2 года назад за 15 минут набросал сайт <http://lenkom21.ru/>, который до сих пор жив, правда редко заполняется.
> Использовалась стандартная тема и единственный дополнительный плагин - форма обращения старшему по дому.
...
- Что дешевле сопровождать - сайт на CMS или собственноручно написаный движок?
- Насколько сложно сделать свою CMS? (Кстати, ваш ассистент в школе таким баловался ещё на PHP 4. Как хорошо, что этот код не сохранился...)
