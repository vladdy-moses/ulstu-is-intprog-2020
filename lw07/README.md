...
Во-вторых, этот метод взаимодействия с улититами / платформами / серверами является распространённым в профессиональной разработке и администрировании.
...
Иногда в windows трубется первой командой указать букву диска, например, `d:`, а потом уже делать _change directory (cd)_.
...
* _web_ - шаблон для создания простейшего веб-приложения. Есть и другие шаблоны, их пожете поискать самостоятельно или с помощью команды `dotnet new`;
...
Теперь пришло время соединить предудущие пункты.
...
* "Хмм, всё, что начинается с /api, отавайте мне, остальное мне не надо";
...
Можете самостоятельно поискать по коду, как описанные выше строки отбражаются в исходный код на _C#_.

Как видите, пока ничего сложного.

Далее необходимо сопоставить наше веб-прилоежение (по сути это набор обработчиков запросов) с конкретным веб-сервером.
...
* При запросе _/error_ должен возвращаться такой код статуса, который придёт в qyery-строке в параметре _status_. Если не придёт, то отдавать код статуса _200_. Тогда запрос в браузере будет выглядеть примерно как <http://localhost:5000/error?status=403> и в этом случае выводить якобы ошибку о том, что нет доступа к ресурсу.
...
Ссылка на регистрацию работы: <TODO>.
